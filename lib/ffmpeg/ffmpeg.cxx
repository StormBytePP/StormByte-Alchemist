...
FFmpeg::FFmpeg(const unsigned int& film_id, const std::filesystem::path& input_file, const std::optional<Database::Data::film::group>& group): m_film_id(film_id), m_input_file(input_file), m_group(group), m_container("mkv"), m_status(CONVERT_QUEUE) {}

FFmpeg::FFmpeg(unsigned int&& film_id, std::filesystem::path&& input_file, std::optional<Database::Data::film::group>&& group): m_film_id(film_id), m_input_file(std::move(input_file)), m_group(std::move(group)), m_container("mkv"), m_status(CONVERT_QUEUE) {}
