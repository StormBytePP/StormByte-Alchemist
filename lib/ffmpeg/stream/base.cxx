...
Stream::Base::Base(unsigned short stream_id, const std::string& encoder, char type):m_stream_id(stream_id), m_encoder(encoder), m_type(type) {}

Stream::Base::Base(const Base& codec):m_stream_id(codec.m_stream_id), m_encoder(codec.m_encoder), m_type(codec.m_type), m_bitrate(codec.m_bitrate) {}

Stream::Base::Base(Base&& codec) noexcept :m_stream_id(std::move(codec.m_stream_id)), m_encoder(std::move(codec.m_encoder)), m_type(std::move(codec.m_type)), m_bitrate(std::move(codec.m_bitrate)) {}
