...
Color::Color(std::string&& primaries, std::string&& matrix, std::string&& transfer, std::string&& pix_fmt)
:m_prim(std::move(primaries)), m_matrix(std::move(matrix)), m_transfer(std::move(transfer)), m_pix_fmt(std::move(pix_fmt)) {}

const std::string& Color::GetPrimaries() const { return m_prim; }

const std::string& Color::GetMatrix() const { return m_matrix; }

const std::string& Color::GetTransfer() const { return m_transfer; }
