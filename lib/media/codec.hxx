...
namespace Alchemist::Media::Codec {
	enum DLL_PUBLIC Type:unsigned short {
		...
		// Copy special items
		COPY_VIDEO		= std::numeric_limits<unsigned short>::max() - 3,
		COPY_AUDIO		= std::numeric_limits<unsigned short>::max() - 2,
		COPY_IMAGE		= std::numeric_limits<unsigned short>::max() - 1,
		COPY_SUBTITLE	= std::numeric_limits<unsigned short>::max()
	};
