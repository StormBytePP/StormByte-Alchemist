...
const std::filesystem::path File::GetDatabaseFile() const {
	return ExpandEnvironmentVariable(Child("database")->AsString());
}
...
const std::filesystem::path File::GetTmpDirectory() const {
	return ExpandEnvironmentVariable(Child("tmpdir")->AsString());
}
...
const std::filesystem::path File::GetHomePath() {
	#ifdef LINUX
...
const std::filesystem::path File::GetConfPath() {
	#ifdef LINUX
	return GetHomePath() / ".alchemist";
	#else
	return GetHomePath() / "Alchemist";
	#endif
}
...
const std::filesystem::path File::GetFilePath() {
	return GetConfPath() / "config";
}

void File::Initialize() {
	if (!std::filesystem::exists(GetConfPath())) {
		/* If there is no path there is no config neither */
		std::filesystem::create_directory(GetConfPath());
...
const std::string File::ExpandEnvironmentVariable(const std::string& var) {
	#ifdef WINDOWS
	return ExpandEnvironmentVariable(UTF8Decode(var));
	#else
	return std::regex_replace(var, std::regex("~"), GetHomePath().string());
	#endif
}
#ifdef WINDOWS
const std::string File::ExpandEnvironmentVariable(const std::wstring& var) {
...
std::string File::UTF8Encode(const std::wstring& wstr) {
...
std::wstring File::UTF8Decode(const std::string& str) {
