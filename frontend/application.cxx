...
Application::Application(): m_config(new Configuration()), m_status(HALT_ERROR) {}
...
int Application::run(int argc, char** argv) noexcept {
	...
	switch(m_status) {
		case RUN_TASK:
			return m_task->run(m_config) == Task::STATUS::HALT_OK ? 0 : 1;
	...
	try {
		while (counter < argc) {
			...
			if (argument == "-t" || argument == "--test") {
				m_task = &Task::Test::get_instance();
				m_status = RUN_TASK;
				counter++;
			}
			else if (argument == "-d" || argument == "--daemon") {
				m_task = &Task::Daemon::get_instance();
				m_status = RUN_TASK;
				counter++;
			}
				...
				if (++counter < argc) {
					...
					m_task = &Task::Interactive::get_instance();
					m_status = RUN_TASK;
				}
				else
...
void Application::display_header() {
...
void Application::display_compiler_info() {
